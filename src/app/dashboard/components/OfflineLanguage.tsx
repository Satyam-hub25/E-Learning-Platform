...
export default function OfflineLanguageComponent() {
  ...
  const [storageUsed] = useState(455); // MB
  const [totalStorage] = useState(2048); // MB
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'offline' && (
        <div className="space-y-6">
          ...
          >
            <h3 className="text-xl font-semibold mb-4 text-white">💾 Storage Usage</h3>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              <button className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-sm transition-all text-white">
                📥 Download All
              </button>
              <button className="px-4 py-2 bg-red-600 hover:bg-red-700 rounded-lg text-sm transition-all text-white">
                🗑️ Clear Cache
              </button>
            </div>
          </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h4 className="font-semibold text-lg text-white">{lesson.title}</h4>
                  ...
                  <div className="flex flex-col items-end gap-2 ml-4">
                    {lesson.isDownloading ? (
                      ...
                      <div className="flex gap-2">
                        <button className="px-4 py-2 bg-green-600 hover:bg-green-700 rounded-lg text-sm transition-all text-white">
                          ▶️ Play
                        </button>
                        <button 
                          onClick={() => handleDeleteDownload(lesson.id)}
                          className="px-3 py-2 bg-red-600 hover:bg-red-700 rounded-lg text-sm transition-all text-white"
                        >
                          🗑️
                        </button>
                      </div>
                      ...
                      <button 
                        ...
                        className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-sm transition-all text-white"
                      >
                        📥 Download
                      </button>
                    )}
      ...
      {activeTab === 'language' && (
        <div className="space-y-6">
          ...
          >
            <h3 className="text-xl font-semibold mb-4 text-white">🌐 Current Language</h3>
            <div className="flex items-center justify-center gap-3 text-2xl">
              <span>{languages.find(lang => lang.code === selectedLanguage)?.flag}</span>
              <span className="font-semibold text-white">{languages.find(lang => lang.code === selectedLanguage)?.name}</span>
            </div>
            {isTranslating && (
              <div className="mt-4">
          ...
          {/* Language Selection */}
          <div>
            <h3 className="text-xl font-semibold mb-4 text-white">📱 Available Languages</h3>
          ...
          <motion.div
            ...
            className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/10"
          >
            <h3 className="text-xl font-semibold mb-4 text-white">⚙️ Translation Settings</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-medium text-white">Auto-translate new content</div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-medium text-white">Download translations</div>
